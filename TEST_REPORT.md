...
| 6 — Embedded live H.264 stream | `7b25b21`, `e6b79ea`, `c6bb2b3` plus 2026-08-23 recovery/unload evidence | 0.2.0 integration base; helper 0.6.0-dev; no embedded release | Core 2026.8.2 | HAOS 18.2 | Yellow AArch64; reproducible AArch64 Linux build | Owlet Cam 1, user-reported | Not established | **Passed locally:** 163 tests at 86.53% branch-aware coverage; Ruff format/check, mypy, secret scan and release validation passed; two independent helper builds were byte-identical | **Partial:** real changing live video, two simultaneous viewers, idle/reopen, `camera.snapshot`, `camera.record`, automatic settled-reload, active-viewer reload and cold-restart readiness, zero Repairs and no new Owlet/stale-source error | **Passed bounded media path:** Home Assistant rendered real infrared frames before and after an active-viewer reload and produced a valid JPEG plus playable five-second MP4 | **Bounded media path and automatic runtime recovery passed; full Milestone 6 gate incomplete** | Corrected loopback diagnostics: streaming, healthy, 1,147 frames, two consumers, zero reconnects; cold restart and no-viewer reload returned to `ready`; active-viewer reload returned in 4.318 seconds and was `loaded` at the first follow-up, with the dialog retained, no Owlet/demuxing/connection-refused entry, zero Repairs, real video after dialog reopen, and final idle state | Direct Core-namespace FFprobe; exact live profile/bitrate; two-hour and overnight soaks; companion-app; Dream coexistence; physical outage/power-cycle/Yellow reboot; direct Core PID orphan enumeration | Raw H.264 failed with missing DTS and was replaced by timestamped MPEG-TS. A dialog retained across entity recreation does not automatically resubscribe; close and reopen it to view the replacement entity |
