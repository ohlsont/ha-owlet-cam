...
| 6 — Embedded live H.264 stream | `7b25b21`, `e6b79ea`, `c6bb2b3` plus 2026-08-23 recovery evidence | 0.2.0 integration base; helper 0.6.0-dev; no embedded release | Core 2026.8.2 | HAOS 18.2 | Yellow AArch64; reproducible AArch64 Linux build | Owlet Cam 1, user-reported | Not established | **Passed locally:** 161 tests at 85.81% branch-aware coverage; Ruff and mypy passed; two independent helper builds were byte-identical | **Partial:** real changing live video, two simultaneous viewers, idle/reopen, `camera.snapshot`, `camera.record`, automatic settled-reload and cold-restart readiness, zero Repairs and no new Owlet error | **Passed bounded media path:** Home Assistant rendered real infrared frames and produced a valid JPEG plus playable five-second MP4 | **Bounded media path and automatic runtime recovery passed; full Milestone 6 gate incomplete** | Corrected loopback diagnostics: streaming, healthy, 1,147 frames, two consumers, zero reconnects; cold restart and no-viewer reload automatically returned to `ready`; final state after active-viewer reload: ready, idle, zero consumers, loopback binding; final cold-start log search empty | Direct Core-namespace FFprobe; exact live profile/bitrate; two-hour and overnight soaks; companion-app; Dream coexistence; physical outage/power-cycle/Yellow reboot; direct Core PID orphan enumeration | Raw H.264 failed with missing DTS and was replaced by timestamped MPEG-TS. Active-viewer reload still waits for Home Assistant's retained WebRTC consumer to close; afterward automatic runtime recovery succeeds |
...
- Local validation after the recovery correction: `161 passed`, 85.81%
  ...
  private-path rejection and multi-consumer fan-out. Recovery tests require an
  exact private consent marker, defer cold-start work until Core is running, and
  schedule one restore per setup/reload.
