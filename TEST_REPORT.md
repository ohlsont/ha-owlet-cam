...
| 6a — Experimental incoming audio | Pending gate commit | 0.8.0; helper 0.8.0 | Local Home Assistant 2026.8.2 test environment | Unperformed | macOS ARM64 tests; reproducible AArch64 Linux helper | No real camera used yet | Not established | **Passed locally:** current full suite remains above the 85% branch-aware threshold; Ruff, mypy, secret scan and release validation pass. Separate inherited-pipe delivery, raw-AAC ADTS wrapping, AAC MPEG-TS PID/PES, two-client fan-out, unsupported/malformed audio fallback and strict FFprobe audio parsing are covered | Unperformed | Unperformed | **Implementation and synthetic-media gate passed; real-camera audio gate pending** | Reproducible proprietary-free helper archive SHA-256 `91b35d95…f755`. FFmpeg 7.1 independently recognized and decoded integration-generated MPEG-TS as H.264 plus AAC-LC, 8000 Hz, mono. Audio is disabled by default and diagnostics retain only codec/status/counters/safe codes | Deploy helper/integration to Yellow, enable audio, confirm real codec/frame counters, Core-local FFprobe audio track, audible Home Assistant playback, reload/unload reaping and video-only behavior on forced audio failure | The pinned independent Owlet implementation reports raw AAC-LC at 8 kHz mono, but this project does not claim the user's camera audio works until real frames and audible playback are observed |
