...
| 8 — Hardening and stable release | `ffa8e1d` plus follow-ups | 0.7.0 | Local and GitHub Home Assistant stable/beta matrix on 2026-08-26 | Not applicable locally | macOS ARM64; AArch64 helper target; cross-platform Python preparer target | Owlet Cam 1 only for prior embedded evidence | Not established | **Passed:** 268 local tests at 85.16% branch-aware coverage; Ruff format/check, mypy, secret scan and release inspection passed. GitHub stable and beta suites, Hassfest and quality passed. The dispatched AArch64 workflow built the archive twice reproducibly, inspected it and uploaded the artifact. Compact-package tests cover deterministic zipapp/package builds, fixed-member/hash/key validation, adb split collection, private-config apkeep invocation, upload and round-trip extraction | No Milestone 8 deployment performed | No new real-camera test performed | **In progress; stable gate not passed** | GitHub runs `32956936771` (stable/beta success), `32956936786` (private-repository validation success) and `32956581716` (AArch64 helper success). A deterministic dependency-free `owlet-cam-prepare.pyz` now produces a strict minimal `.owletcam` input outside Home Assistant. The apkeep token remains only in its mode-0600 config and is absent from argv/output | Real adb export and real apkeep download, upload of a real `.owletcam` package on Yellow, HACS Action on the public repository, HACS installation, real external bridge gate, wrong-password real reauth, multiple-user testing and stable release | Repository intentionally remains private; HACS requires public repository access. Full APK upload remains a development fallback. Do not tag 1.0.0 or claim HACS/stable success |
