...
| 8 — Hardening and stable release | `e75ab86` plus this evidence update | 0.7.0 | Local and GitHub Home Assistant stable/beta matrix; Yellow Core 2026.8.3 on 2026-08-26 | HAOS 18.2 | macOS ARM64; Yellow AArch64; cross-platform Python preparer target | Owlet Cam 1, user-reported | Not established | **Passed:** 268 local tests at 85.14% branch-aware coverage; Ruff format/check, mypy, secret scan and release inspection passed. GitHub stable and beta suites, Hassfest and quality passed for the sidebar-removal follow-up. The dispatched AArch64 workflow built the archive twice reproducibly, inspected it and uploaded the artifact. Compact-package tests cover deterministic zipapp/package builds, fixed-member/hash/key validation, adb split collection, private-config apkeep invocation, native Home Assistant file selection, private setup storage and round-trip extraction | **Passed for the deployed follow-up:** checksum-verified source installed with `userfiles` preserved; config check and Core restart succeeded; entry loaded; the sidebar panel was absent; exact helper 0.7.0 restored runtime `ready`; all five native libraries passed; zero Repairs and zero current structured Owlet system-log entries | No new frame or live-stream probe was required; prior Milestones 4–7 retain real-camera evidence | **Deployed follow-up passed; stable gate remains in progress** | Source commit `e75ab86`; source archive SHA-256 `624f7b…6446`; CI helper run `32956581716`, artifact SHA-256 `318c96…b5`. The private repository prevented anonymous exact-release download, so the same inspected CI artifact was installed manually with its fixed allowlist and atomic swap. Diagnostics reported helper `0.7.0`, `sdk_key_found=true`, five compatible AArch64 libraries, helper started 1/reaped 1/all reaped, and no safe error. Deployment staging, source rollback, three obsolete runtime backups and macOS metadata were removed; active runtime and extracted user-supplied material were retained. Temporary key-only SSH was then fully closed | Real adb export and real apkeep download, upload of a real `.owletcam` package through the new setup flow on Yellow, HACS Action on the public repository, HACS installation, real external bridge gate, wrong-password real reauth, multiple-user testing and stable release | Repository intentionally remains private; HACS and anonymous helper-release download require public repository access. Historical logs still contain earlier development stream/import errors, but the current structured Owlet system log is empty. Do not tag 1.0.0 or claim HACS/stable success |
