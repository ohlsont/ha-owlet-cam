...
| 4 — Embedded connection and frame probe | `165093f` plus 2026-08-22 live evidence | Development-only; no embedded release | Core 2026.8.2 | HAOS 18.2 | Yellow AArch64; Android 15 ARM64 emulator on ARM64 macOS | Owlet Cam 1, user-reported | Not established | Current suite 126 passed at 86.60% coverage; Ruff and mypy passed; rebuilt session-mode helper passed ELF/runtime archive validation | **Passed frame-receipt sub-gate:** three separate 100-frame probes with SPS/PPS/IDR, 1920×1080 and clean shutdown | **Passed on Yellow:** real H.264 received in three bounded probes, including rapid successive sessions; user confirmed Dream live video afterward | **Yellow H.264 receipt and post-probe Dream recovery passed; full coexistence gate remains open** | Yellow results: 450,104/450,118/451,325 bytes; 7/7/7 SPS/PPS/IDR each; 14.196/14.341/13.599 FPS; 81/71/262 ms first frame; clean shutdown each; no new Core exit, Repair or Owlet error; user-observed Dream recovery | Deploy rebuilt session-mode helper; official-app-open-before-probe behavior; direct/relay session-mode observation; direct process-list/orphan capture | Docker/OrbStack IOTC timed out with `-13`; deployed helper predates the new session-mode field, so direct versus relay is still unknown |
...
- A follow-up clean-room change adds only the bounded session-mode enum (`p2p`,
  ...
  It has not yet been deployed to Yellow. MCP also cannot provide a direct
  process-list capture. Official-app-open-before-probe behavior and live
  session-mode observation are therefore still required before the complete
  Milestone 4 gate can be marked passed.
