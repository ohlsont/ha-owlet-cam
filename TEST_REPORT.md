...
| 6a — Experimental incoming audio | `ad308a3` through `14e6481` | 0.8.0; helper 0.8.0 | Core 2026.8.3 | HAOS 18.2 | Yellow AArch64; reproducible AArch64 Linux helper | Owlet Cam 1, user-reported | Not established | **Passed locally:** 284 tests at 85.12% branch-aware coverage; Ruff format/check and mypy passed. Separate inherited-pipe delivery, native ADTS/LOAS detection, bare-AAC wrapping, AAC MPEG-TS PID/PES, two-client fan-out, unsupported/malformed audio fallback and strict/redacted FFprobe parsing are covered | **Passed bounded Core media probe:** checksum-verified 0.8.0 helper and source loaded; audio option enabled; configuration checks and restarts succeeded; runtime restored automatically; zero active Repairs | **Passed decode path:** real camera supplied 64 audio frames / 49,152 bytes during the successful bounded session; Core-local FFprobe decoded/counts H.264 plus AAC-LC, 8000 Hz, mono | **Real Core audio track passed; audible Home Assistant client confirmation pending** | Proprietary-free helper archive SHA-256 `91b35d95…f755`. Successful probe: H.264 Baseline 1920×1080 at 15 FPS, 119 counted video frames, 63 counted AAC frames, 473.8 kbit/s combined bounded bitrate. Native framing was safely classified as ADTS despite public Kalay codec ID `0x88`. Helper accounting ended started 2/reaped 2/all reaped, zero forced kills, no safe error | Audible Home Assistant browser playback; deliberate audio-pipe failure on Yellow; formal reload/unload with an actively audible client | Earlier LATM and double-ADTS interpretations were rejected by real FFprobe evidence before this passing result. Audio remains disabled by default and experimental; no audible success is claimed yet |
...
- Audible browser playback is deliberately not marked passed until the user
  unmutes a Home Assistant live view and confirms room sound.
