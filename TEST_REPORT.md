...
- The user checked Dream during the same validation period and neither
  temperature nor humidity was displayed there. This supports treating the two
  values as currently unavailable from the tested camera/firmware while leaving
  protocol support in place for devices that do report them.
...
- The normal Home Assistant connection selector now lists and defaults to
  ...
  separately hosted compatibility fallback. The earlier Milestone 0
  development mode was subsequently removed from release runtime code.
