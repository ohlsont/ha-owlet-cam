...
- A post-fix Core restart, a real file-picker upload/progress run, and the
  confirmation-gated delete → missing-APK Repair → re-upload resolution cycle
  remain unperformed. Browser automation could not obtain Home Assistant's
  file chooser, so no live upload claim is made. Milestone 7 is not yet
  accepted.
