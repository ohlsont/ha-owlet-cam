...
| 8 — Hardening and stable release | Pending gate commit | 0.7.0 | Local Home Assistant 2026.8.2 | Not applicable locally | macOS ARM64; AArch64 helper target | Owlet Cam 1 only for prior embedded evidence | Not established | **Passed locally:** 259 tests at 85.34% branch-aware coverage; Ruff format/check and mypy passed. Focused release tests cover deterministic metadata, archive safety, checksums, version/architecture enforcement and partial cleanup | No Milestone 8 deployment performed | No new real-camera test performed | **In progress; stable gate not passed** | HACS/Hassfest fixes, stable/beta CI matrix, reproducible AArch64 helper workflow, exact-version downloader, checksums, SPDX SBOM, licence manifest, changelog notes and release-archive inspection are implemented locally | GitHub Actions rerun, actual helper workflow asset, HACS installation, public repository, real external bridge gate, wrong-password real reauth, multiple-user testing and stable release | Repository intentionally remains private; do not tag 1.0.0 or claim HACS/stable success |
