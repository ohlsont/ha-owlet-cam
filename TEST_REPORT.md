...
| 8 — Hardening and stable release | `11d5e45` plus this evidence update | 0.7.0 | Local and GitHub Home Assistant stable/beta matrix on 2026-08-26 | Not applicable locally | macOS ARM64; AArch64 helper target; cross-platform Python preparer target | Owlet Cam 1 only for prior embedded evidence | Not established | **Passed:** 268 local tests at 85.14% branch-aware coverage; Ruff format/check, mypy, secret scan and release inspection passed. GitHub stable and beta suites, Hassfest and quality passed for the sidebar-removal follow-up. The dispatched AArch64 workflow built the archive twice reproducibly, inspected it and uploaded the artifact. Compact-package tests cover deterministic zipapp/package builds, fixed-member/hash/key validation, adb split collection, private-config apkeep invocation, native Home Assistant file selection, private setup storage and round-trip extraction | No Milestone 8 deployment performed; the sidebar-removal follow-up is intentionally not deployed while the user is travelling | No new real-camera test performed | **In progress; stable gate not passed** | GitHub runs `32969225794` (current stable/beta success), `32969225531` (current Hassfest/quality/private-readiness success) and `32956581716` (AArch64 helper success). Embedded setup now accepts `.owletcam` through Home Assistant's public FileSelector/file-upload path; Reconfigure can replace it or separately confirm proprietary deletion. The permanent custom sidebar panel, frontend asset and private runtime HTTP API are removed. The file upload ID is transient and absent from config-entry data | Real adb export and real apkeep download, upload of a real `.owletcam` package through the new setup flow on Yellow, HACS Action on the public repository, HACS installation, real external bridge gate, wrong-password real reauth, multiple-user testing and stable release | Repository intentionally remains private; HACS requires public repository access. The current UI accepts only compact `.owletcam` packages; complete APK/APKM/XAPK/ZIP files are preparer inputs. Do not tag 1.0.0 or claim HACS/stable success |
...
- Local result: 268 tests passed with 85.14% branch-aware coverage. Ruff and
  ...
  private-repository readiness job. No Yellow deployment or real-camera test
  was performed for this follow-up; prior Milestone 7 panel evidence below is
  retained as historical evidence and is not a claim about the current UI.
