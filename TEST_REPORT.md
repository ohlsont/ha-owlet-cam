...
| 7b — Experimental embedded room sensors | Pending gate commit | 0.9.0 unreleased | Local Home Assistant 2026.8.2 | Not applicable locally | macOS ARM64 host; reproducible Yellow AArch64 helper | Owlet Cam 1, user-reported | Not established | **Passed locally:** 292 tests at 85.22% branch-aware coverage; Ruff, mypy, freestanding helper compilation, reproducible AArch64 packaging and release-asset inspection passed | Pending | Pending | **Implementation complete; real Yellow values not yet claimed** | Fixed-size inherited telemetry-pipe tests cover all five fields, partial masks, range rejection, cached entity state and video isolation. Proprietary-free helper archive SHA-256 `61a60ae…c8ef` | Yellow deployment; comparison with plausible room conditions; stream continuity and child reaping with telemetry enabled | Sensor values update only while the embedded live camera session is active; option remains off by default until the real-device check passes |
