...
| 2 — External bridge | Pending gate commit | 0.7.0 | Local tests use Home Assistant 2026.8.2; minimum 2024.11.0 | Not applicable locally | macOS ARM64 test host; bridge architecture independent | No real bridge camera tested | Not established | **Passed locally:** current full 259-test suite includes current-schema bridge API parsing, authentication, partial capabilities, multi-camera config flow, reauth/reconfigure/options, coordinator setup/unload, camera source/snapshot and metric entities | Unperformed | Unperformed | **Implementation complete; real bridge gate unperformed** | Clean-room adapter matches reference commit `132620a85ff422b451e52fdbf2076abb3975e9ec`; it ignores returned UID/AuthKey/password fields, derives the advertised RTSP URL and requests `units=metric` | Real bridge H.264, dashboard video/still, service snapshot/record, outage/recovery, two viewers and resource measurements | The reference bridge has no API version field and no licence; no source was copied. Stable release remains blocked until the real bridge gate is exercised or explicitly left unsupported |
...
| 6a — Experimental incoming audio | `ad308a3` through `e8ac526` plus the default-on follow-up | 0.8.0; helper 0.8.0 | Core 2026.8.3 | HAOS 18.2 | Yellow AArch64; reproducible AArch64 Linux helper | Owlet Cam 1, user-reported | Not established | **Passed locally:** 285 tests at 85.14% branch-aware coverage; Ruff format/check and mypy passed. Separate inherited-pipe delivery, native ADTS/LOAS detection, bare-AAC wrapping, AAC MPEG-TS PID/PES, two-client fan-out, unsupported/malformed audio fallback and strict/redacted FFprobe parsing are covered | **Passed bounded Core media probe:** checksum-verified 0.8.0 helper and source loaded; audio option enabled; configuration checks and restarts succeeded; runtime restored automatically; zero active Repairs | **Passed real media and audible playback:** real camera supplied 64 audio frames / 49,152 bytes during the successful bounded session; Core-local FFprobe decoded/counts H.264 plus AAC-LC, 8000 Hz, mono; the user then confirmed audible room audio in Home Assistant's live camera view | **Experimental incoming-audio gate passed for the tested Owlet Cam 1 / Yellow setup** | Proprietary-free helper archive SHA-256 `91b35d95…f755`. Successful probe: H.264 Baseline 1920×1080 at 15 FPS, 119 counted video frames, 63 counted AAC frames, 473.8 kbit/s combined bounded bitrate. Native framing was safely classified as ADTS despite public Kalay codec ID `0x88`. Helper accounting ended started 2/reaped 2/all reaped, zero forced kills, no safe error. User audible confirmation recorded 2026-08-27. Incoming audio is now the default for new/unset options; explicit off choices remain respected | Deliberate audio-pipe failure on Yellow; formal reload/unload with an actively audible client; other camera models and client platforms | Earlier LATM and double-ADTS interpretations were rejected by real FFprobe evidence before this passing result. Audio remains experimental outside the single tested setup and can be disabled for video-only operation |
| 7 — Upload UI and integration polish | `a7074c6` plus this gate commit | 0.2.0 integration base; helper 0.6.0-dev; no embedded release | Local 2026.8.2; Yellow Core 2026.8.3 | HAOS 18.2 | Local macOS ARM64; Yellow AArch64 | Owlet Cam 1, user-reported | Not established | **Passed locally:** 228 tests at 85.14% branch-aware coverage; Ruff, mypy, JSON, JavaScript syntax, release validation, secret scan and whitespace checks passed | **Passed:** administrator panel, real file-picker upload, default archive deletion, confirmation-gated proprietary-file deletion, missing-APK Repair creation/resolution, persisted runtime, real frame/stream probes, redacted diagnostics, post-fix restart recovery and child reaping were exercised | **Passed bounded probes:** 100 real frames at 1920×1080 with SPS/PPS/IDR and clean shutdown; FFprobe saw H.264 Baseline 1920×1080 at 15 FPS | **Complete Milestone 7 Yellow gate passed** | Deletion removed proprietary material, set safe `missing_apk`, created the expected Repair and left no active helper. The panel accepted a user-selected 154,946,466-byte XAPK; the Repair cleared, runtime probing restored SDK-key presence and five compatible AArch64 libraries, the uploaded archive was deleted by default, runtime returned `ready`, helper accounting ended started 3/reaped 3 with zero forced kills, and the current structured Owlet log was empty | Formal visual timing of the progress bar; upload success and exact received byte count were independently verified in diagnostics | External bridge and HACS publication remain separately deferred |
| 8 — Hardening and stable release | `e75ab86` plus this evidence update | 0.8.0 unreleased | Local and GitHub Home Assistant stable/beta matrix; Yellow Core 2026.8.3 on 2026-08-26 | HAOS 18.2 | macOS ARM64; Yellow AArch64; cross-platform Python preparer target | Owlet Cam 1, user-reported | Not established | **Passed:** 290 local tests at 85.14% branch-aware coverage; Ruff format/check, mypy, secret scan and release inspection passed. GitHub stable and beta suites, Hassfest and quality passed for the earlier sidebar-removal follow-up. The dispatched AArch64 workflow built the archive twice reproducibly, inspected it and uploaded the artifact. Compact-package tests now cover deterministic zipapp/package builds, fixed-member/hash/key validation, adb split collection, private-config Google Play invocation, credential-free APKPure ARM64 selection, unknown-native-code rejection, native Home Assistant file selection, private setup storage and round-trip extraction | **Passed for the deployed follow-up:** checksum-verified source installed with `userfiles` preserved; config check and Core restart succeeded; entry loaded; the sidebar panel was absent; exact helper 0.7.0 restored runtime `ready`; all five native libraries passed; zero Repairs and zero current structured Owlet system-log entries | No new frame or live-stream probe was required; prior Milestones 4–7 retain real-camera evidence | **Deployed follow-up passed; stable gate remains in progress** | Source commit `e75ab86`; source archive SHA-256 `624f7b…6446`; CI helper run `32956581716`, artifact SHA-256 `318c96…b5`. The private repository prevented anonymous exact-release download, so the same inspected CI artifact was installed manually with its fixed allowlist and atomic swap. The new APKPure trust gate independently accepted the retained ignored Dream 3.36.0 ARM64 bundle's five-library hash set without serializing its SDK key. Diagnostics reported helper `0.7.0`, `sdk_key_found=true`, five compatible AArch64 libraries, helper started 1/reaped 1/all reaped, and no safe error. Deployment staging, source rollback, three obsolete runtime backups and macOS metadata were removed; active runtime and extracted user-supplied material were retained. Temporary key-only SSH was then fully closed | Fresh external APKPure download through the released zipapp, real adb export, upload of a real `.owletcam` package through the new setup flow on Yellow, HACS Action on the public repository, HACS installation, real external bridge gate, wrong-password real reauth, multiple-user testing and stable release | Repository intentionally remains private; HACS and anonymous helper-release download require public repository access. Historical logs still contain earlier development stream/import errors, but the current structured Owlet system log is empty. Do not tag 1.0.0 or claim HACS/stable success |

| 7b — Experimental embedded room sensors | `b085440`, released by `8edff5e` | 0.9.1 private release | Core 2026.8.3 | HAOS 18.2 | macOS ARM64 host; reproducible Yellow AArch64 helper | Owlet Cam 1, user-reported | Not established | **Passed locally:** 292 tests at 85.22% branch-aware coverage; Ruff, mypy, freestanding helper compilation, reproducible AArch64 packaging and release-asset inspection passed | **Passed deployment and bounded-session safety; partial sensor coverage:** exact 0.9.1 integration/helper loaded, runtime reached `ready`, entities appeared, existing video/audio probe passed, zero active Repairs and zero current Owlet system-log entries | **Partially passed:** sound `0 dB`, illuminance `17 lx` and Wi-Fi `-49 dBm` were received from the real camera; temperature and humidity remained `unknown`. The sound value was not independently correlated and is recorded only as received | **Three fields populated; temperature/humidity and value-correlation gates remain open** | Release source SHA-256 `14d41605…015d`; proprietary-free helper SHA-256 `e0b58681…f955`. Same bounded session reported H.264 Baseline 1920×1080 at 15 FPS, 675.3 kbit/s and AAC (Kalay `0x88`). Option persisted through the official options flow. Userfiles were preserved; deployment staging and rollback copies were removed; temporary key-only SSH was fully closed | Correlating Cam 1 sensor values with the Dream app; longer sensor session; other camera models | Sensor values update only while an embedded session is active. The implementation rejects out-of-range telemetry instead of publishing guessed values; temperature/humidity may remain unavailable on the tested Cam 1 |
...
- GitHub release `v0.9.1` passed its release workflow. The Yellow independently
  matched SHA-256 `14d41605696ba00b3e33bca8387430e9ec62b953819dbde3782792b4a174015d`
  for the integration archive and
  `e0b5868174c20a1df0b8fe9c80702a04c896e6e9bfb1300ab2d5999af73f9550`
  for the proprietary-free AArch64 helper before installation.
- The complete existing `userfiles` tree was moved intact across the source
  swap. Core's configuration check was valid, Core restarted, the config entry
  loaded, runtime status reached `ready`, and helper version reported `0.9.1`.
- Enabling **Experimental local sensors** through the official options flow
  created all five entities. One bounded real-camera session produced sound
  `0 dB`, illuminance `17 lx`, and Wi-Fi `-49 dBm`. Temperature and humidity
  remained `unknown`; no scale conversion or success claim was invented.
- The same session retained H.264 Baseline 1920×1080 at 15 FPS and 675.3 kbit/s,
  plus the previously validated AAC path. This is evidence that the telemetry
  side channel did not interrupt video or audio for the bounded run.
- Home Assistant returned zero current Owlet system-log entries and zero active
  Repairs after the probe. The exact uploaded archives, staging directory,
  source/runtime rollback copies, and temporary deployment script were deleted.
  Active integration files and user-supplied runtime material were retained.
  The temporary SSH key and port mapping were removed; password remained empty,
  TCP forwarding remained disabled, and an independent port-22222 check failed
  closed as expected.

## Credential-free APKPure preparer validation — 2026-08-27

- The standalone preparer now exposes an `apkpure` command that invokes
...
- The path compares the complete five-library name/SHA-256 set before writing a
  ...
  3.40.0 installation. The SDK key and its hash are not part of the trust
  record.
- A local read-only revalidation of the retained ignored APKPure bundle reported
  only package `com.owletcare.sleep`, version 3.36.0, `arm64-v8a`, five
  libraries, SDK-key presence `true`, and trusted-set match `true`. Temporary
  extracted files were removed automatically; no proprietary value was printed
  or added to Git.
- Full local validation passed: 290 tests at 85.14% branch-aware coverage, Ruff
  format/check, mypy for 43 source files, secret scan for 98 files, release
  metadata validation, deterministic preparer build/help execution, release
  archive inspection, and whitespace validation. A fresh external APKPure
  download through the newly built zipapp remains explicitly unperformed.

## Experimental incoming-audio validation — 2026-08-26

- The exact proprietary-free helper archive used on Yellow has SHA-256
  `91b35d9543933dec39410d9a802b2f91cea4919243ea2368673907cb412af755`.
  It contains the clean-room helper and open-source runtime only; user-supplied
  libraries, application files, SDK key, and camera credentials are absent.
- The isolated helper delivered video and audio through separate inherited file
  descriptors. Credentials remained startup JSON on stdin and were neither
  command-line arguments nor diagnostic fields. Audio failure handling remained
  independent from the H.264 producer.
- The first real probe received AAC frames but rejected the initial assumed
  LATM packetization. A bounded allowlist was added for codec type/name,
  profile, dimensions, rates, counts, channel count, and container only. It
  excludes source URLs, paths, tags, stderr, credentials, and media bytes.
- Redacted observations first showed AAC-LATM with no audio configuration, then
  AAC-LC mono with an unknown rate after an extra ADTS header. A framing-only
  classifier finally established that the camera's `0x88` frames already begin
  with ADTS. Preserving that native header produced the passing result.
- The final Core-local FFprobe result was H.264 Baseline level 4.0, 1920×1080,
  15 FPS, 119 counted video frames, AAC-LC 8000 Hz mono, 63 counted audio
  frames, MPEG-TS, and 473.8 kbit/s bounded combined bitrate. Integration
  counters recorded 122 video frames and 64 audio frames / 49,152 audio bytes.
- The stream returned to idle after the probe. Helper accounting reported two
  starts, two reaps, `all_reaped=true`, zero forced kills, and no safe error.
  Home Assistant reported zero active Repairs. The error-log endpoint returned
  no current log resource, consistent with an empty current error log.
- On 2026-08-27, the user opened and unmuted the Home Assistant live camera view
  and confirmed that room audio was audible. This completes the bounded incoming
  audio gate for the tested Owlet Cam 1 / Home Assistant Yellow setup only.

## Native configuration upload follow-up — 2026-08-26

- Home Assistant Core's public file selector and `file_upload` component are now
  the only UI upload path. A real test HTTP upload is consumed by the embedded
  setup flow, copied to a generated mode-0600 filename, and removed from Home
  Assistant's temporary upload store. The file ID is not persisted in the
  config entry.
- Reconfigure tests cover keeping the existing package, replacing it, rejecting
  simultaneous replace/delete requests, requiring a separate destructive
  confirmation, successful deletion, an unavailable runtime manager, and a
  redacted runtime deletion failure.
- The custom sidebar panel, JavaScript module, and integration-specific runtime
  HTTP views are absent. Runtime and frame/stream controls remain tested as
  disabled-by-default diagnostic button entities.
- Local result: 268 tests passed with 85.14% branch-aware coverage. Ruff and
  mypy passed. GitHub run `32969225794` passed on Home Assistant stable and beta;
  run `32969225531` passed Hassfest, quality, release/secret checks and the
  private-repository readiness job.
- Source commit `e75ab86` was deployed to Yellow with SHA-256 verification while
  preserving `userfiles`. Home Assistant's configuration check passed, Core
  restarted, the config entry returned to `loaded`, and the
  `owlet-cam-runtime` panel was absent from the live panel registry.
- Because the repository remains private, Core could not anonymously download
  a `v0.7.0` release asset. The exact proprietary-free AArch64 artifact from
  successful CI run `32956581716` was therefore downloaded through the
  authenticated developer checkout, inspected, independently hashed as
  `318c966f5c3dad72f6ce189788db8487dfa3fe8adf902d825ade757f716146b5`
  locally and on Yellow, allowlist-checked, and atomically installed. Runtime
  revalidation returned `ready`, helper `0.7.0`, SDK-key presence, five
  compatible AArch64 libraries, no safe error, and started 1/reaped 1/all
  reaped helper accounting.
- Active Repairs and current structured Owlet system-log entries were empty.
  Deployment staging, the source rollback, three obsolete helper backups and
  macOS metadata files were deleted after validation. The active helper plus
  extracted user-supplied libraries and SDK key were retained. Temporary SSH
  was returned to zero keys, empty password, forwarding disabled and no host
  port mapping; an independent TCP check found port 22222 closed.
- No new real-camera frame or streaming test was performed for this UI-only
  follow-up; prior Milestones 4–7 real-media evidence below is retained as
  historical evidence and is not a claim about the current setup UI.
...
- The user completed the real confirmation-gated delete in the panel. Safe
  diagnostics immediately showed `not_prepared`, `missing_apk`, application
  status `deleted`, no proprietary files, no libraries or SDK key, an inactive
  helper, started 2/reaped 2 and zero forced kills. The expected
  `missing_apk` warning Repair appeared and the current structured Owlet log
  remained empty.
- The user then selected the local Dream XAPK in the real panel file picker.
  Home Assistant recorded an accepted upload of 154,946,466 bytes and the
  missing-application Repair cleared. A bounded runtime-probe button press took
  about 18 seconds and returned `ready`, helper 0.6.0-dev, `arm64-v8a`, SDK-key
  presence and all five compatible libraries. The default retention policy
  deleted the uploaded archive after extraction. Final helper accounting was
  started 3/reaped 3, all reaped, zero forced kills; active Repairs and current
  structured Owlet system-log entries were empty. Milestone 7 is accepted on
  Yellow. Visual timing of the progress indicator was not independently
  recorded, but the real file selection, complete byte count and successful
  processing were verified.
