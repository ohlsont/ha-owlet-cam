...
| 8 — Hardening and stable release | `ffa8e1d` plus private-gate follow-up | 0.7.0 | Local and GitHub Home Assistant stable/beta matrix on 2026-08-26 | Not applicable locally | macOS ARM64; AArch64 helper target | Owlet Cam 1 only for prior embedded evidence | Not established | **Passed:** 259 local tests at 85.34% branch-aware coverage; Ruff format/check, mypy, secret scan and release inspection passed. GitHub stable and beta suites, Hassfest and quality passed. The dispatched AArch64 workflow built the archive twice reproducibly, inspected it and uploaded the artifact | No Milestone 8 deployment performed | No new real-camera test performed | **In progress; stable gate not passed** | GitHub runs `32956531318` (stable/beta success) and `32956581716` (AArch64 helper success). HACS Action run `32956531390` could not read repository metadata while the repository was private; the public-only job remains configured and is skipped honestly until publication | HACS Action on the public repository, HACS installation, real external bridge gate, wrong-password real reauth, multiple-user testing and stable release | Repository intentionally remains private; HACS requires public repository access. Do not tag 1.0.0 or claim HACS/stable success |
